...
export function SearchPage(){
 const {t}=useTranslation(); const [params,setParams]=useSearchParams(); const initial=params.get('q')||''; const [q,setQ]=useState(initial);
 ...
 useEffect(()=>{if(!term){setItems([]);return}setLoading(true);setError('');setPage(1);searchTitles(term,1).then(r=>{setItems(r.items);setTotalPages(r.totalPages)}).catch(e=>setError(e.message)).finally(()=>setLoading(false))},[term]);
 const more=()=>{const next=page+1;setLoading(true);searchTitles(term,next).then(r=>{setItems(v=>[...v,...r.items]);setPage(next);setTotalPages(r.totalPages)}).catch(e=>setError(e.message)).finally(()=>setLoading(false))};
