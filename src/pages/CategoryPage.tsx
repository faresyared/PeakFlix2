...
export function CategoryPage(){
 ...
 const labels:Record<string,Record<string,string>>={
   ...
   'turkish-drama': { en: 'Turkish Drama', ar: 'الدراما التركية', es: 'Drama Turco', ja: 'トルコドラマ', fr: 'Drame Turc', it: 'Drammi Turchi', de: 'Türkisches Drama' }
 };

 useEffect(()=>{
   setItems([]);setPage(1);setLoading(true);
   getCategory(category,1).then(r=>{setItems(r.items);setTotalPages(r.totalPages)}).catch(e=>setError(e.message)).finally(()=>setLoading(false))
 ...
 return <div className="page-shell"><div className="page-banner"><span>EXPLORE TMDB</span><h1>{currentLabel}</h1></div>{error&&<div className="empty-state"><h2>{error}</h2></div>}<div className="catalog-grid">{items.map(x=><MediaCard key={x.id} item={x}/>)}</div>{loading&&<div className="load-status">Loading...</div>}{!loading&&page<totalPages&&<button className="load-more" onClick={more}>Load more</button>}</div>;
}